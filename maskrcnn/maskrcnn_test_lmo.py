...
def main():
    ...
    data_dir = '/data2/yifeis/pose/stablepose_maskrcnn_data_release/lmo/test'
    dataset_test = PennFudanDataset(data_dir, get_transform(train=False))
    data_loader_test = torch.utils.data.DataLoader(
        dataset_test, batch_size=3, shuffle=False, num_workers=5,
        collate_fn=utils.collate_fn)

    num_classes = 100
    model = get_model_instance_segmentation(num_classes)
    model.load_state_dict(torch.load('./log_lmo/network.pth'))
    model.to(device)
